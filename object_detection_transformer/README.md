...
- `scripts/start_stack.(sh|ps1)` launches the API backend and Streamlit dashboard together so you can control the full pipeline from one page.
...
- Opening the dashboard by itself is enough: if the backend is offline, the UI now exposes a **Start local backend** button that spins up the FastAPI server inline so you can upload data, train, and run inference without leaving the page.
...
### Single command stack launch
Start both the FastAPI backend and Streamlit dashboard together (uses `FUSIONNET_API_TOKEN` if set):
```pwsh
./scripts/start_stack.ps1
```
Use the dashboard to upload a YOLO dataset (zip), register it, trigger training, and run inference without leaving the UI.
...
```
The dashboard includes dataset upload/registration, training controls, and image inference wired to the bundled FastAPI backend.
...
- For TPU training on Kaggle, copy `kaggle_fusionnet_notebook.py` into a notebook (set accelerator to **TPU v5e B**), provide data via `FUSIONNET_KAGGLE_DATASET=<owner/dataset>` or an uploaded ZIP at `/kaggle/input`, and set `FUSIONNET_BACKEND_URL` / `FUSIONNET_API_TOKEN` to sync checkpoints back to the Codespaces backend.
